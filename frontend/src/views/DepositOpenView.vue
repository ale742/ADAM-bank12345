...
const startFaceID = () => {
    ...
    setTimeout(() => {
        try {
            ...
            triggerStatus('success', 'Успешно!', 'Ваш депозит открыт. Переходим в управление.');
            setTimeout(() => router.push('/deposits'), 2000);
        } catch (e) {
            isVerifying.value = false;
...
.active-opt {
    background: linear-gradient(135deg, #005bb5 0%, #00124d 100%);
    box-shadow: 0 15px 30px rgba(0, 78, 146, 0.4) !important;
...
.scan-line-ios-blue {
    ...
    animation: scanIos 2.5s infinite ease-in-out;
}
