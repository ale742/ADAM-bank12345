<template>
  <div class="page-wrapper">
    ...
    <!-- Контент -->
    <div class="container pt-5 mt-4 pb-5" v-if="auth.user">
      ...
      <div class="card-container mb-4 animate__animated animate__fadeInDown">
        <div class="bank-card p-4 shadow-lg text-white position-relative overflow-hidden" 
            ...
            <div v-if="auth.user.isBlocked" class="frozen-overlay d-flex flex-column align-items-center justify-content-center">
                ...
                <h5 class="fw-bold m-0 ls-1 text-info text-uppercase">Карта заморожена</h5>
            </div>
            ...
            <div class="position-relative z-2 mb-4 d-flex align-items-center justify-content-between">
                <h4 class="font-monospace text-shadow mb-0 card-number">
                    {{ showCardNumber && !auth.user.isBlocked ? auth.user.card_number : maskedCardNumber }}
                </h4>
            ...
            <div class="d-flex justify-content-between align-items-end position-relative z-1">
                ...
                <div class="text-end date-area-fixed">
                    ...
                    <span class="fw-bold font-monospace z-2 position-relative">{{ auth.user.card_exp }}</span>
                </div>
            </div>
      ...
      <div class="d-flex justify-content-between align-items-center mb-3 ps-2">
          <h6 class="fw-bold text-muted m-0">Ваши продукты</h6>
          <small v-if="hasDeposit && auth.user.deposits.length < 5" class="text-primary fw-bold cursor-pointer" @click="$router.push('/open-deposit')">+ Открыть</small>
      </div>

      <div class="list-group rounded-4 shadow-sm border-0 mb-4 overflow-hidden bg-white">
        
        <!-- ЕСЛИ ЕСТЬ ДЕПОЗИТЫ - ВЫВОДИМ СПИСКОМ -->
        <template v-if="hasDeposit">
            <div v-for="dep in auth.user.deposits" :key="dep.id" 
                class="list-group-item p-3 border-0 border-bottom action-row" 
                @click="handleDepositClick(dep.id)">
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h6 class="mb-0 fw-bold">{{ dep.title }}</h6>
                            <small class="text-success fw-bold">
                                {{ dep.isAmountHidden ? '•••••• ₸' : formatMoney(dep.amount) + ' ₸' }}
                            </small>
                        </div>
                    </div>
        ...
        <!-- ЕСЛИ ДЕПОЗИТОВ НЕТ - ОДНА КНОПКА ОТКРЫТИЯ -->
        <div v-else class="list-group-item p-3 border-0 action-row" @click="handleDepositClick()">
            <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                    <div class="icon-circle bg-success bg-opacity-10 text-success me-3"><i class="bi bi-piggy-bank-fill fs-5"></i></div>
                    <div>
                        <h6 class="mb-0 fw-bold">Депозиты</h6>
                        <small class="text-muted">Ставка до 22%</small>
                    </div>
                </div>
                ...
                <div v-if="activeModal === 'limits'">
                    ...
                    <div class="mb-4" :class="{'opacity-50': auth.user.isBlocked}">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div class="form-check form-switch"><input class="form-check-input" type="checkbox" v-model="localLimits.internet" :disabled="auth.user.isBlocked"></div>
                        </div>
    ...
    <Transition name="fade">
      <div v-if="isVerifying" class="face-id-ios-overlay">
          <div class="face-id-container">
              ...
              <p class="text-white-50 small">Подтверждение биометрии...</p>
          </div>
      </div>
    </Transition>
...
const confirmDialog = reactive({ visible: false, title: '', message: '', icon: '', type: '', confirmText: 'Ок', cancelText: 'Отмена', showCancel: true, action: null });
...
const openConfirm = (title, message, icon, type, confText, cancelText, showCancel, action) => {
    Object.assign(confirmDialog, { visible: true, title, message, icon, type, confirmText: confText || 'Ок', cancelText: cancelText || 'Отмена', showCancel, action });
};
const closeConfirm = () => { confirmDialog.visible = false; };

// ХЕНДЛЕРЫ
const handleDepositClick = (id = null) => {
    if (auth.user.isBlocked) return openConfirm('Карта заблокирована', 'Сначала разблокируйте её.', 'bi-snow', 'danger', 'Понятно', '', false, closeConfirm);
    if (id) router.push(`/deposits/${id}`);
    else router.push('/open-deposit');
};
...
const handleInstallmentClick = () => {
    if (auth.user.isBlocked) return openConfirm('Карта заблокирована', 'Разблокируйте карту.', 'bi-snow', 'danger', 'Понятно', '', false, closeConfirm);
    router.push('/shop');
};
...
const triggerConfirmBlock = () => {
    ...
    openConfirm(
        isBlocking ? 'Заморозка карты' : 'Разморозка карты', 
...
.eye-btn-card { cursor: pointer; font-size: 1.3rem; color: white; opacity: 0.8; }

/* MASTERCARD LOGO */
.card-logo-fixed { bottom: 15px; right: 15px; display: flex; position: absolute; z-index: 1; }
...
.date-area-fixed { padding-right: 65px; }
...
/* FACE ID iOS СТИЛЬ (СИНИЙ) */
.face-id-ios-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.92); z-index: 6000; display: flex; align-items: center; justify-content: center; text-align: center; }
